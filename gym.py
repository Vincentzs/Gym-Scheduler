...
class Instructor:
    ...
    def can_teach(self, workout_class: WorkoutClass) -> bool:
        ...
        True
        """
        certificates_needed = workout_class.get_required_certificates()
        required = 0
        if len(certificates_needed) == 0:
            return True
        for certificates in certificates_needed:
            if certificates in self._certificates:
                required += 1
        return required == len(certificates_needed)
...
class Gym:
    ...
    def add_instructor(self, instructor: Instructor) -> bool:
        ...
        if instructor_id not in self._instructors:
            # to access the instructor name:
            # gym name._instructors[instructor.get_id].name
            # e.g. ac._instructors[1].name
    ...
    def add_workout_class(self, workout_class: WorkoutClass) -> bool:
        ...
        True
        """
        workout_name = workout_class.get_name()
        if workout_name not in self._workouts:
            self._workouts[workout_name] = workout_class
            return True
        return False
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        if time_point in self._schedule:
            for room in self._schedule[time_point]:
                if instructor in self._schedule[time_point][room] or \
                        room == room_name:
                    return False
        if instructor.can_teach(workout):
    ...
    def register(self, dict_rooms: datetime, client: str, workout_name: str) \
            -> bool:
        ...
        False
        """
        dict_rooms = self._schedule[dict_rooms]
        for room in dict_rooms:
            if client in dict_rooms[room][2]:
                return False
            if dict_rooms[room][1].get_name() == workout_name \
                    and len(dict_rooms[room][2]) < self._rooms[room]:
                dict_rooms[room][2].append(client)
                return True
            return False
    ...
    def offerings_at(self, time_point: datetime) -> List[Tuple[str, str, str]]:
        ...
        if time_point in schedule:
            for room in schedule[time_point]:
                gym_class = schedule[time_point][room]
                offerings.append((gym_class[0].name, gym_class[1].get_name(),
                                  room))
        return offerings
    ...
    def instructor_hours(self, time1: datetime, time2: datetime) -> \
        ...
        for instructor in self._instructors:
            total_hours[instructor] = 0

        for time_point in schedule:
            if time1 <= time_point <= time2:
                for room in schedule[time_point]:
                    instructor_id = schedule[time_point][room][0].get_id()
                    total_hours[instructor_id] += 1
        return total_hours
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        """
        total_hours = self.instructor_hours(time1, time2)
        instructors = self._instructors
        pay = []
        for instructor in instructors:
            bonus_multiple = \
                self._instructors[instructor].get_num_certificates()
            amount = total_hours[instructor] * base_rate + \
                     total_hours[instructor] * bonus_multiple * BONUS_RATE
            pay.append((instructor, self._instructors[instructor].name, \
                        total_hours[instructor],
                        amount))
        pay.sort()
        return pay
